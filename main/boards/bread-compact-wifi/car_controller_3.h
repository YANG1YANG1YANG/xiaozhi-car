...
private:
    ...
    bool is_moving_ = false, is_being_moved_=true,is_returning_=false;
    ...
    static void motor_control_task(void* arg) {
        ...
        while (1) {
            ...
            if (self->is_moving_ && fabsf(self->target_speed_left_) < 0.5f && fabsf(self->target_speed_right_) < 0.5f) {
            ...
            if(self->is_being_moved_==false && self->is_moving_==false && (delta1!=0 || delta2!=0)){
                self->is_being_moved_=true;
                gpio_set_level(self->stby_pin_,0);
            }
            if(self->is_being_moved_==true && delta1==0 && delta2==0){
                wait_cnt++;
            }
            if(wait_cnt>=15){
            ...
            if(self->is_returning_==true && fabsf(self->left_cumul_)<=10 && fabsf(self->right_cumul_)<=10){
                self->is_moving_=false;
                self->is_returning_=false;
                self->left_cumul_=0;
                self->right_cumul_=0;
            }
            if(self->is_straight_&&self->is_returning_==false){
                self->position_pid_straight();
            }
            if(self->is_returning_==true){
            ...
            if (++print_cnt % 50 == 0) { // 每 100 次循环（约 1 秒）打印一次
